 ...
 * NOTE: This manual stream-based extractor is a last-resort fallback and is less reliable
 * than proper layout-aware PDF text extraction (e.g., pdfjs-dist). Flagged for future
 * replacement with a layout-preserving extraction library rather than primary use.
 */
    ...
    while ((match = streamRegex.exec(bufferStr)) !== null) {
      ...
      try {
        ...
        for (const m of tjMatches) {
          const clean = m.replace(/^\(/, '').replace(/\)\s*Tj$/, '').replace(/\\([()\\])/g, '$1').trim();
        ...
        for (const m of arrayMatches) {
          ...
          const combined = innerStrings
            .map(s => s.replace(/^\(/, '').replace(/\)$/, '').replace(/\\([()\\])/g, '$1'))
            .join('');
          const trimmed = combined.trim();
          if (trimmed && trimmed.length > 1) textChunks.push(trimmed);
        }
      } catch {
